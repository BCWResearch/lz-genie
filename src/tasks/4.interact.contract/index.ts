...
export default {
    ...
    run: async (_backCb: Function) => {
        ...
        if (selectedFunctionType) {
            const fnList = filterFunctions(abi, selectedFunctionType as 'getter' | 'setter');
            const selectedFunction = await InquirerUtils.handlePrompt(
                fnList.reduce((acc, f) => {
                    acc[f.name] = {
                        ...
                        tag: f.name,
                        run: async () => {
                            console.log(`Interacting with ${selectedFunctionType} function: ${f.name}`);
                        }
                    };
                    return acc;
            ...
            if (selectedFunction) {
                ...
                HardhatTaskUtil.generateTaskFile(tasksDir, onlyContractName, selectedFunction, inputs);
                HardhatTaskUtil.updateIndexFile(tasksDir, `import './${onlyContractName}_${selectedFunction}';`);
